...
static int nacl_irt_ioctl (int fd, unsigned long int request, int arg) {
  return NACL_SYSCALL (ioctl) (fd, request, arg);
}
...
int (*__nacl_irt_ioctl) (int fd, unsigned long int request, int arg);
